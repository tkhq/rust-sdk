...
impl TurnkeyClient {
    ...
    pub async fn process_activity(
        ...
        loop {
            ...
            let parsed = serde_json::from_str::<ActivityResponse>(&res_text).map_err(|e| TurnkeyClientError::Decode(res_text, e))?;
            ...
            match activity.status {
                ...
                ActivityStatus::Unspecified | ActivityStatus::Created | ActivityStatus::Rejected => {
                    return Err(TurnkeyClientError::UnexpectedActivityStatus(activity.status.as_str_name().to_string()))
                },
            }
        }
    }
...
mod test {
    ...
    fn test_create_sub_organization_response_parsing() {
        ...
        assert_eq!(parsed.activity.clone().unwrap().id, "019660f7-801d-75d8-a40e-e4f69944b711");
        ...
        match inner {
            Inner::CreateSubOrganizationResultV7(res) => {
                assert_eq!(res.sub_organization_id, "d047f3a2-6c66-40ee-ab71-95f8a0148fe3")
            },
            _other => {
