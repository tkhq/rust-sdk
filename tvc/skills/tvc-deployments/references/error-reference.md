...
| `not_found` | HTTP 404, or an OK response with an empty resource. | Confirm the `--app-id` / `--deploy-id`. There is no `deploy list`, but `tvc app status --app-id <APP_ID>` returns the app's `deployments[]` with their IDs once the app has a live deployment (before that the backend answers with a 500 `api_error`, and an unapproved deployment is not listed), and `tvc app list` gives the app IDs. When a deployment lookup 404s the message reads `cannot find deployment <id> in organization <org>`, where `<org>` is `'<alias>' (<id>)` under a `tvc login` profile or the bare ID under env auth. The credentials are scoped to `<org>`, and the most common cause is that the deployment belongs to a different organization: switch org (`tvc login --org <alias-or-id>` for a profile, `TVC_ORG_ID` for env auth) before assuming the ID is wrong. |
...
- A 404-style "not found" can also mean "exists but has no state yet" (e.g. `deploy get-status` before replicas appear returns `replicas: null` rather than erroring, but a truly unknown ID errors as `not_found`). Distinguish by whether you hold a valid ID, and check the organization first when the message names one.
