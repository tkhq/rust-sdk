...
pub struct Args {
    /// Path to the quorum key metadata file, which includes the share to re-encrypt.
    /// Assumed to be encrypted by the operator key.
    #[arg(long, env = "QUORUM_KEY_METADATA", value_name = "PATH")]
    ...
    #[arg(long, env = "PROVISION_BUNDLE", value_name = "PATH")]
    ...
    #[arg(long, env = "OUTPUT_PATH", value_name = "PATH")]
...
pub async fn run(args: Args) -> anyhow::Result<()> {
    ...
    let quorum_key_metadata = read_quorum_key_metadata(&args.quorum_key_metadata).await?;
...
async fn read_quorum_key_metadata(path: &Path) -> anyhow::Result<QuorumKeyMetadata> {
    let contents = tokio::fs::read(path).await.with_context(|| {
        format!(
            "failed to read quorum key metadata file: {}",
            path.display()
        )
    })?;
    serde_json::from_slice(&contents).with_context(|| {
        format!(
            "failed to parse quorum key metadata file: {}",
            path.display()
        )
    })
}
...
mod tests {
    ...
    #[tokio::test]
    async fn dangerous_skip_allows_quorum_key_mismatch() {
        let metadata_quorum_pair = P256Pair::generate().unwrap();
        let manifest_quorum_pair = P256Pair::generate().unwrap();
        let operator_pair = P256Pair::generate().unwrap();
        let operator_member = QuorumMember {
